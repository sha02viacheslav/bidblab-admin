...
export class QuestionDetailComponent implements OnInit, OnDestroy {
  ...
  canReport(answerId, answererId){
    return (
      !(this.authenticationService.getUser()._id === answererId) &&
      !this.reports.some(
          report =>
  ...
  async deleteQuestion() {
    try {
      const res = (await this.commonService.deleteQuestion(
        this.question._id
      )) as any;
  ...
  async deleteAnswer(answerId) {
    try {
      const res = (await this.commonService.deleteAnswer(
        this.question._id,
        answerId
      )) as any;
      this.socketsService.notify('deletedData', {
      ...
      const index = this.question.answers.findIndex(
        currentAnswer => currentAnswer._id === answerId
      );
      if (index !== -1) {
