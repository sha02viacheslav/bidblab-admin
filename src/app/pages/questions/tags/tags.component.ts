...
export class TagsComponent implements OnInit {
  ...
  addTag() {
    if (this.searchForm.valid) {
      this.submitted = true;
      this.questionsService.addTag(this.searchForm.value).subscribe((res: any) => {
  ...
  updateTag(tagId){
    if (this.updateForm.valid) {
      this.submitted = true;
      this.questionsService.updateTag(tagId, this.updateForm.value).subscribe((res: any) => {
        this.submitted = false;
        if(res.data) {
          this.snackBar.open(res.msg, 'Dismiss', {duration: 1500});
          this.getTags();
  ...
  public finalDeleteTags(elementIds) {
    if (elementIds.length) {
      if (confirm("Are you sure to delete tags")) {
        this.questionsService.deleteTags(elementIds).subscribe((res: any) => {
          this.getTags();
        });
      }
    }
